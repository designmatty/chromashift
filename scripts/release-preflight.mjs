...
async function verifyArtifacts(version) {
  ...
  requireMatch(manifest, /sha512:\s+\S+/, 'Update manifest sha512')
  requireMatch(manifest, /size:\s+\d+/, 'Update manifest size')
}
...
globalThis.console.log(
  `Release preflight passed: version=${version}, tag=${options.tag ?? 'not-required'}, architecture=x64, artifacts=${options.artifacts ? 'verified' : 'not-requested'}`
)
